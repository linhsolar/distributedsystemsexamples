...
int main(int argc, char **argv) {
  ...
  printf("SERVER - Pls enter to start bind():");
  fgets(temp,BUFSIZE,stdin);
  
  if (bind(listenfd, (struct sockaddr *) &serveraddr, 
	   sizeof(serveraddr)) < 0) 
    error("ERROR on binding");
  printf("SERVER - Pls. enter to start listen():");
  fgets(temp,BUFSIZE,stdin);
  ...
  while (1) {
    ...
    printf("SERVER - Pls. enter to start accept()");
   fgets(temp,BUFSIZE,stdin);
    ...
    bzero(buf, BUFSIZE);
 printf("SERVER - Pls. enter to start read()");
 fgets(temp,BUFSIZE,stdin);
    n = read(connfd, buf, BUFSIZE);
    if (n < 0) 
      error("ERROR reading from socket");
    printf("server received %d bytes: %s", n, buf);
  printf("SERVER - Pls. enter to start write()");
  fgets(temp,BUFSIZE,stdin) ;  
